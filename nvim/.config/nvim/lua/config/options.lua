...
vim.opt.spelllang = { "en", "pt-BR" } -- Set language for spellschecking
...
vim.opt.matchtime = 2 -- How long show matching bracket
...
vim.opt.autoread = true -- Auto-reload file in change outside
vim.opt.autowrite = false -- Don't auto-save on some splits
...
local undodir = "~/.local/share/nvim/undordir" -- Undo directory path
vim.opt.undodir = vim.fn.expand(undodir) -- Expand to full path
local undordir_path = vim.fn.expand(undodir)
if vim.fn.isdirectory(undordir_path) == 0 then
  vim.fn.mkdir(undordir_path, "p") -- Create if not exists
end
...
vim.opt.foldexpr = "v:lua.vim.treesitter.foldeexpr()" -- Use treesitter for folding
vim.opt.foldlevel = 99 -- keep all folds open by default

-- Split Behavior
vim.opt.splitbelow = true -- Horizontal splits open below
vim.opt.splitright = true -- Vertical slipts open to the right

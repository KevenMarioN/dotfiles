...
if ok then
  ...
  vim.keymap.set("n", "<leader>fl", fzf.blines, { desc = "Fuzzy in opened file " })
  ...
  vim.keymap.set('n', '<leader>fr', fzf.lsp_references, { desc = "FZF Show all references to the symbol under the cursor" })
  vim.keymap.set('n', '<leader>ft', fzf.lsp_typedefs, { desc = "FZF Jump to the type definition of symbol under the cursor" })
